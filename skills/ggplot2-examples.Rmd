...
First off, "Introductory Data Visualization" is kind of a lie. Most introductory data visualization lessons start with R's Base Graphics methods like ```plot()```. I'm skipping past that and jumping right into plotting with ```ggplot2()```. The reasons are simple:
...
```{r}
plot <- ggplot(scatter.df,aes(x=x,y=y)) + geom_point()
